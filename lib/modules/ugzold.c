...
static avssize_t orig_read(ave *v, void *devinfo, char *buf, avsize_t nbyte)
{
    ...
    return av_read(v, gunz->fh, buf, nbyte);
}
			    ...
			    avsize_t nbyte)
{
    ...
    return av_write(v, gunz->rf->outfd, buf, nbyte);
}
...
static avoff_t orig_lseek(ave *v, void *devinfo, avoff_t offset, int whence)
{
    ...
    return av_lseek(v, gunz->fh, offset, whence);
}
...
static int read_sure(ave *v, int fh, char *buf, avsize_t nbyte)
{
    ...
    rres = av_read(v, fh, buf, nbyte);
    if(rres == -1) return -1;
  
    if(rres != (avssize_t) nbyte) {
        av_log(AVLOG_ERROR, "UGZ: Error: Premature end of file");
        v->errn = EIO;
        return -1;
    }
...
static int read_gzip_header(ave *v, struct gunzip *gunz)
{
    ...
    if(buf[0] != GZMAGIC1 || buf[1] != GZMAGIC2) {
        av_log(AVLOG_ERROR, "UGZ: Error: File not in GZIP format");
        v->errn = EIO;
        return -1;
    }
    ...
    if(method != METHOD_DEFLATE) {
        av_log(AVLOG_ERROR, "UGZ: Error: File compression is not DEFLATE");
        v->errn = EIO;
        return -1;
    }

    if(flags & GZFL_RESERVED) {
        av_log(AVLOG_ERROR, "UGZ: Error: Unknown flags");
        v->errn = EIO;
        return -1;
    }
    ...
    if(av_lseek(v, fh, -4, AVSEEK_END) == -1) return -1;
    ...
    if(av_lseek(v, fh, 0, AVSEEK_SET) == -1) return -1;
...
static void gunzip_free(void *obj)
{
    ...
    av_free_vpath(gunz->savepath);
    if(gunz->vf != AVNULL) av_vfile_destroy(DUMMYV, gunz->vf);
    if(gunz->fh != -1) av_close(DUMMYV, gunz->fh);
		      ...
		      
{
    ...
    gunz->savepath = av_copy_vpath(v, path);
    if(gunz->savepath == AVNULL) return -1;

    gunz->fh = av_open(v, BASE(path), AVO_RDONLY, 0);
    if(gunz->fh == -1) return -1;

    av_registerfd(gunz->fh);
    gunz->file_usage = 1;
...
static struct gunzip *get_gunzip(ave *v, vpath *path, int flags, int mode)
{
    ...
    if(av_stat(v, BASE(path), &stbuf, AVFS_FLAG_NOSIZE) == -1) {
        if(v->errn == ENOENT && (flags & AVO_CREAT)) {
            int tmpfd;

            tmpfd = av_open(v, BASE(path), 
                              AVO_WRONLY | AVO_TRUNC | AVO_CREAT | 
                              (flags & AVO_EXCL), mode);
            if(tmpfd == -1) return AVNULL;
            res = av_fstat(v, tmpfd, &stbuf, AVFS_FLAG_NOSIZE);
            av_close(DUMMYV, tmpfd);
        ...
        if(flags & (AVO_ACCMODE | AVO_TRUNC) && 
           av_access(v, BASE(path), AVW_OK) == -1) return AVNULL;
    }

    if(flags & AVO_TRUNC) empty = 1;

    major = av_get_vdev(path)->major;
    minor = av_getminor(major, stbuf.dev, stbuf.ino);
    cobj = av_cache_find(v, major, minor, gunzip_valid, 
                           (void *) &stbuf.mtime);

    if(cobj == AVNULL) return AVNULL;

    if(cobj->obj != AVNULL) {
        av_cache_op(cobj, COBJ_LOCK);
        gunz = (struct gunzip *) cobj->obj;
        if(gunz->file_usage == 0) {
            gunz->fh = av_open(v, BASE(path), AVO_RDONLY, 0);
            if(gunz->fh == -1 ||
               av_lseek(v, gunz->fh, gunz->lastoffset, AVSEEK_SET) == -1) {
                av_cache_op(cobj, COBJ_FREE);
                return AVNULL;
            }
        }
        gunz->file_usage ++;
        av_cache_op(cobj, COBJ_UNLOCK);
    }
    else {
        AV_NEW(v, gunz);
        if(gunz == AVNULL) {
        ...
        gunz->cobj = cobj;
        gunz->stbuf = stbuf;
        gunz->stbuf.dev = av_vmakedev(major, minor);
    ...
    if(empty) {
        av_cache_op(cobj, COBJ_LOCK);
        res = av_vfile_truncate(v, gunz->vf, 0);
        gunz->dirty = 1;
        gunz->stbuf.size = 0;
        av_cache_op(cobj, COBJ_UNLOCK);
...
static int write_gzip_header(ave *v, struct gunzip *gunz)
{
    ...
    wres = av_write(v, gunz->rf->outfd, buf, GZHEADER_SIZE);
    if(wres == -1) return -1;
...
static int write_gzip_footer(ave *v, struct gunzip *gunz, avqbyte crc)
{
    ...
    wres = av_write(v, gunz->rf->outfd, buf, GZFOOTER_SIZE);
    if(wres == -1) return -1;
...
static int flush_gzip(ave *v, struct gunzip *gunz)
{
    ...
    gunz->rf = av_get_replacement(v, BASE(gunz->savepath), 0);
    if(gunz->rf == AVNULL) return -1;
  
    res = write_gzip_header(v, gunz);
    if(res != -1)
        res = av_zfilt_flush(v, gunz->vf, &crc);
    if(res != -1)
        res = write_gzip_footer(v, gunz, crc);

    if(res == -1) {
        av_del_replacement(gunz->rf);
        return -1;
    }
  
    res = av_replace_file(v, gunz->rf);
...
static int gunzip_close(ave *v, void *devinfo)
{
    ...
    av_cache_op(gunz->cobj, COBJ_LOCK);
    if(gunz->dirty) {
    ...
    gunz->file_usage --;
    if(gunz->file_usage == 0) {
        gunz->lastoffset = av_lseek(DUMMYV, gunz->fh, 0, AVSEEK_CUR);
        av_close(DUMMYV, gunz->fh);
        gunz->fh = -1;
    }
    av_cache_op(gunz->cobj, COBJ_UNLOCK);

    av_cache_op(gunz->cobj, COBJ_RELEASE);
    av_free(di);
...
static avssize_t gunzip_read(ave *v, void *devinfo, char *buf, avsize_t nbyte)
{
    ...
    av_cache_op(di->gunz->cobj, COBJ_LOCK);  
    res = av_vfile_read(v, di->gunz->vf, buf, di->ptr, nbyte);
    av_cache_op(di->gunz->cobj, COBJ_UNLOCK);  
			      ...
			      avsize_t nbyte)
{
    ...
    av_cache_op(gunz->cobj, COBJ_LOCK);
    res = av_vfile_write(v, gunz->vf, buf, di->ptr, nbyte);
    if(res != -1) {
        gunz->dirty = 1;
...
static int gunzip_ftruncate(ave *v, void *devinfo, avoff_t length)
{
    ...
    av_cache_op(gunz->cobj, COBJ_LOCK);
    res = av_vfile_truncate(v, gunz->vf, length);
    if(res != -1) {
        gunz->dirty = 1;
        if(gunz->vf->size != -1) gunz->stbuf.size = gunz->vf->size;
    }
...
static avoff_t gunzip_lseek(ave *v, void *devinfo, avoff_t offset, int whence)
{
    ...
    return av_generic_lseek(v, &di->ptr, di->gunz->stbuf.size, offset,
                              whence);
}
...
static int gunzip_access(ave *v, vpath *path, int amode)
{
    ...
    return av_access(v, BASE(path), amode);
}
...
static int gunzip_chmod(ave *v, vpath *path, avmode_t mode)
{
    ...
    return av_chmod(v, BASE(path), mode);
}
                        ...
                        int deref)
{
    ...
    return av_chown(v, BASE(path), uid, gid);
}
...
static int gunzip_fchmod(ave *v, void *devinfo, avmode_t mode)
{
    ...
    return av_fchmod(v, di->gunz->fh, mode);
}
...
static int gunzip_fchown(ave *v, void *devinfo, avuid_t uid, avgid_t gid)
{
    ...
    return av_fchown(v, di->gunz->fh, uid, gid);
}


extern int av_init_module_ugz(ave *v);

int av_init_module_ugz(ave *v)
{
    ...
    vdev = av_new_vdev(v, "ugz", gunzip_exts, AV_VER);
